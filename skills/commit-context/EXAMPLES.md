...
> `9a1b2c3` on `main` by dev: "rotate refresh tokens". Linked to session `7f3a9c2` "Auth refresh rework" (14 obs). The retry loop handles a token revoked mid-flight, per the session's high-importance observations.
...
> `1122aab` predates agent session linking, so there is no recorded session. From `git show`: it added `validateScope` to enforce per-token scopes. I can show the full diff if useful.
...
Take the SHA, run `memory_commit_lookup`, present commit plus linked session in the same shape as example 1.
