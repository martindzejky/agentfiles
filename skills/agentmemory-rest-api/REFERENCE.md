...
Generated from `src/triggers/api.ts`. Do not edit the block below by hand. Upstream regenerates with `npm run skills:gen`.

<!-- AUTOGEN:rest START - generated by scripts/skills/generate.ts, do not edit by hand -->

The REST API is the main HTTP API. All paths are under `http://localhost:3111` (override with `--port`). When `AGENTMEMORY_SECRET` is set, send `Authorization: Bearer $AGENTMEMORY_SECRET`; localhost is otherwise open.

Registered endpoints (Cursor-irrelevant host bridges omitted):
