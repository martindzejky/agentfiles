...
Generated by scanning `src/` for `AGENTMEMORY_*` usage. Do not edit the block below by hand. Upstream regenerates with `npm run skills:gen`. Internal markers ending in two underscores are excluded.

<!-- AUTOGEN:env START - generated by scripts/skills/generate.ts, do not edit by hand -->

Configuration is read from the environment and from `~/.agentmemory/.env` (no `export` prefix). Recognized variables (Cursor-irrelevant host flags omitted):
