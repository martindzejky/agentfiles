...
If it is not clear what message to use, or what ticket numbers to use, ask the user or stop and report if running autonomously. If the jira ticket number is known but the external ticket ID is not, explicitly ask whether the user has an external ticket ID before writing the changelog entry. If the user responds with nothing, do not write any jira ticket number, just the message.
